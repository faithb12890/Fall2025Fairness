...
for X, y in test_loader:
    for idx in range(len(X)):
        ...
        yp = smooth_attr_bool(X[idx], y[idx], model_dnn_2, start_idx=58, num_att=2, n_samples=10)
...
female_total = 0
 ...
 for i in range(len(X)):                       
    sex = torch.argmax(X[i][start_idx:end]) # 0 = male, 1 = female (from one-hot)

    if sex == 0:  # male
        if y.item() == 1:   # positive example
             male_total += 1
            if label == 1:
                tpr_male_list += 1
    else:         # female
    ...
    tpr_male = tpr_male_list/male_total
    tpr_female = tpr_female_list/female_total    '''

print(f"Final smoothed accuracy: {test_acc}")
#print(f"True positive rate male: {tpr_male}")
