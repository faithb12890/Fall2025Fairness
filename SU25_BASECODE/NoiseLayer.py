...
# try to read normalization mu and sigma vectors from row in csv

class NormalNoise(nn.Module):
    '''
    Custom neural net layer to normalize input data and add noise if smooth flag
    is set to true.
    '''
    def __init__(self, sigma=0.2, smooth=False):
        self.sigma = sigma      # Sigma value for noise. 0.2 by default.
        self.smooth = smooth    # Whether or not to smooth inputs. False by default.
        self.smoothvars = torch.zeros(103)
        # Selecting variables to smooth over
        self.smoothvars[0] = 1      # age
        self.smoothvars[26:33] = 1  # marital-status
        self.smoothvars[53:58] = 1  # race
        self.smoothvars[58:60] = 1  # sex
        self.normalmu = torch.tensor([-1.7706e-09,  7.3888e-01,  8.2853e-02,  3.5608e-02,  3.1265e-02,
         ...
         5.9678e-04,  9.9463e-04,  6.2993e-04,  3.3154e-05])   # Normalization vector - means
        
        self.normalsig = torch.tensor([1.0000, 0.4393, 0.2757, 0.1853, 0.1740, 0.2527, 0.2015, 0.0215, 1.0000,
        ...
        0.0575, 0.0244, 0.0315, 0.0251, 0.0058])   # Normalization vector - stand devs

    def normalize(self, x):
        '''Normalize the input data'''
        x = x - self.normalmu
        x = torch.div(x,self.normalsig)
        return x

    def smoothing(self, smooth):
        '''Set smoothing variable. Boolean True/False'''
        self.smooth = smooth
    
    def add_noise(self, x):
        '''Add noise to input if self.smooth is true. Adapted from LWRS code'''
        if self.smooth == True:
            epsilon = self.sigma * torch.randn_like(x)
            epsilon = epsilon*self.smoothvars
            return x + epsilon
        return x
    
    def forward (self, x):
        '''Forwarding function. Normalizes input, and will always go to noising
        function which handles whether or not to add noise.'''
        x = self.normalize(x)
        x = self.add_noise(x)
